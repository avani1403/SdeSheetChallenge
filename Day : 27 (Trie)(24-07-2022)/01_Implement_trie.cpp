...
public:
    ...
    bool search(string &word, int pos = 0) {
        ...
        else if(word.length() == pos) return false;
        else if(next[word[pos]-'a'] == 0) return false;
        else if(chr[word[pos]-'a'] == 0) return false;
        return next[word[pos]-'a']->search(word, pos+1);
    }

    /** Returns if there is any word in the trie that starts with the given prefix. */
    bool startsWith(string &prefix, int pos = 0) {
        if(prefix.length() == pos) return true;
        else if(next[prefix[pos]-'a'] == 0) return false;
        else if(chr[prefix[pos]-'a'] == 0) return false;
